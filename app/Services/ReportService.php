...
class ReportService
{
    ...
    public function getCloseReport( string $start, string $end, string $type, $user_id = null, $categories_id = null )
    {
        switch ( $type ) {
            case 'products_report':
                $products = $this->getProductsReports(
                    start: $start,
                    end: $end,
                    user_id: $user_id,
                    categories_id: $categories_id
            ...
            case 'categories_report':
            case 'categories_summary':
                $categories =  $this->getCategoryCloseReports(
                    start: $start,
                    end: $end,
                    user_id: $user_id,
                    categories_id: $categories_id
    ...
    public function getCategoryCloseReports( $start, $end, $orderAttribute = 'name', $orderDirection = 'desc', $user_id = null, $categories_id = [] )
    {
        $request = Order::PaymentStatusIn( [Order::PAYMENT_PAID,Order::PAYMENT_REFUNDED, Order::PAYMENT_PARTIALLY_REFUNDED] )
            ->from( $start )
            ->to( $end );
        ...
        if ( ! empty( $user_id ) ) {
            $request = $request->where( 'register_id', $user_id );
        }
         ...
         * summary
         */
        $summary = $this->getCloseSummary( $orders, $user_id, $start, $end );
